...
class VarEsPlotter:
    ...
    def plot_var_es_comparison(
        ...
        title: str = "VaR and ES Comparison by Method",
        ...
        ax.set_title(f'{title} (Confidence: {confidence_level*100:.0f}%)', 
    ...
    def plot_var_breach_analysis(
        ...
        if len(breaches) > 0:
            ax.scatter(breaches.index, breaches.values, 
                      color='red', s=50, zorder=5, label='Breaches')
        ...
        ax.text(0.02, 0.98, f'Breaches: {breach_count} ({breach_rate:.2f}%)',
