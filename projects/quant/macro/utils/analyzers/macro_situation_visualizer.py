...
class MacroSituationVisualizer:
    ...
    def plot_macro_situation(
        ...
        if 'levels' in yield_curve:
            self.yield_curve_plotter.plot_yield_curve(yield_curve['levels'], ax=ax1)
        ...
        if inflation:
            ...
            for key, value in inflation.items():
                if key.endswith('_change_1y') and isinstance(value, (int, float)):
                    name = key.replace('_change_1y', '').upper()
        ...
        if sentiment:
            ...
            Nivel de miedo:           {sentiment.get('fear_level', 'N/A')}
            Fortaleza del dólar:      {sentiment.get('dollar_strength', 'N/A')}
            Demanda de refugio:       {sentiment.get('safe_haven', 'N/A')}
            """
